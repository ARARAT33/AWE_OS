...
use super::abi::{Syscall, SyscallResult, ERR_INVALID_ARGUMENT, ERR_OK, ERR_PERMISSION};
use crate::process::{ProcessDescriptor, ProcessState};

/// Deterministic syscall gate. Architecture-specific trap code should decode
/// registers and pass only the syscall number and arguments here. The gate
/// performs validation before any privileged operation is attempted.
...
mod tests {
    ...
    fn ctx() -> SyscallContext<'static> {
        static mut PROCESS: ProcessDescriptor = ProcessDescriptor {
            id: ProcessId(1), state: ProcessState::Running,
            budget: ResourceBudget { cpu_ticks: 10, memory_bytes: 4096, ipc_messages: 2 },
        };
        // SAFETY: tests are single-threaded and exclusively own the static.
        SyscallContext { process: unsafe { &mut PROCESS } }
    }
