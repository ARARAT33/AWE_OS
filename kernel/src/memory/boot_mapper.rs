...
impl<'a> BootMapper<'a> {
    ...
    pub fn identity_map_range(
        ...
        if length == 0 || (start & 0xfff) != 0 || (length & 0xfff) != 0 {
            return Err(BootMapError::InvalidRange);
        }
        let end = start.checked_add(length).ok_or(BootMapError::Overflow)?;
        let pages = (end - start) / 4096;
        if pages > 512 {
            return Err(BootMapError::TableCapacity);
        }
        let mut i = 0usize;
        while i < pages as usize {
            let physical = start
                .checked_add((i as u64) * 4096)
                .ok_or(BootMapError::Overflow)?;
            self.mapper
                .map(i, physical, flags)
                .map_err(BootMapError::Map)?;
            i += 1;
        }
        Ok(i)
    }
...
mod tests {
    ...
    fn maps_a_small_identity_window() {
        ...
        assert_eq!(
            mapper
                .identity_map_range(0x1000, 0x3000, PageFlags::WRITABLE)
                .unwrap(),
            3
        );
    ...
    fn rejects_overflow_and_unaligned_ranges() {
        ...
        let mut mapper = BootMapper::new(&mut table);
        assert_eq!(
            mapper.identity_map_range(0x1001, 0x1000, PageFlags::empty()),
            Err(BootMapError::InvalidRange)
        );
        assert_eq!(
            mapper.identity_map_range(u64::MAX - 0xfff, 0x2000, PageFlags::empty()),
            Err(BootMapError::Overflow)
        );
    }
    #[test]
    fn rejects_more_than_one_table() {
        ...
        let mut mapper = BootMapper::new(&mut table);
        assert_eq!(
            mapper.identity_map_range(0, 513 * 4096, PageFlags::PRESENT),
            Err(BootMapError::TableCapacity)
        );
    }
}
