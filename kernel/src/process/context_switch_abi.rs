...
impl SwitchPlan {
    ...
    pub const fn next_rip(&self) -> u64 {
        self.next.rip
    }
    pub const fn next_rsp(&self) -> u64 {
        self.next.rsp
    }
    pub const fn target(&self) -> &SwitchFrame {
        &self.next
    }
}
...
mod tests {
    ...
    #[test]
    fn plan_contains_target() {
        let p = build_plan(&frame(0x400000, 0x800000), &frame(0x500000, 0x900000)).unwrap();
        assert_eq!(p.next_rip(), 0x500000);
        assert_eq!(p.next_rsp(), 0x900000);
        assert_eq!(p.target().rip, 0x500000);
    }
    #[test]
    fn invalid_frames_rejected() {
        assert_eq!(
            build_plan(&frame(0, 0x800000), &frame(0x500000, 0x900000)),
            Err(SwitchError::InvalidCurrent)
        );
        assert_eq!(
            build_plan(&frame(0x400000, 0x800000), &frame(0, 0x900000)),
            Err(SwitchError::InvalidNext)
        );
    }
    #[test]
    fn identical_transition_rejected() {
