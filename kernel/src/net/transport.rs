...
pub struct SocketTable<const N: usize = MAX_ENDPOINTS> {
    entries: [Option<SocketEntry>; N],
}
...
impl<const N: usize> SocketTable<N> {
    ...
    pub fn bind(&mut self, endpoint: Endpoint, transport: Transport) -> Result<usize, NetError> {
        if !endpoint.valid()
            || self
                ...
                .iter()
                .flatten()
                .any(|e| e.local == endpoint && e.transport == transport)
        {
    ...
    pub fn get(&self, slot: usize) -> Option<SocketEntry> {
...
pub fn udp_payload(packet: &[u8]) -> Result<(Endpoint, Endpoint, &[u8]), NetError> {
    ...
    if src == 0 || dst == 0 || len < UDP_HEADER || len > packet.len() {
        return Err(NetError::InvalidAddress);
    }
    Ok((
        Endpoint::new(Ipv4Address::UNSPECIFIED, src),
        Endpoint::new(Ipv4Address::UNSPECIFIED, dst),
...
pub fn tcp_header_valid(packet: &[u8]) -> Result<usize, NetError> {
    ...
    if !(TCP_MIN_HEADER..=packet.len()).contains(&offset) {
        return Err(NetError::BufferTooSmall);
    }
    let src = u16::from_be_bytes([packet[0], packet[1]]);
    let dst = u16::from_be_bytes([packet[2], packet[3]]);
...
mod tests {
    ...
    fn checksum_is_deterministic() {
        assert_eq!(internet_checksum(&[0, 1, 0, 2]), 0xfffc);
    }

    #[test]
    fn socket_table_enforces_bounds_and_identity() {
        let mut t = SocketTable::<1>::new();
        let e = Endpoint::new(Ipv4Address::LOOPBACK, 8080);
        let slot = t.bind(e, Transport::Tcp).unwrap();
        assert!(t.bind(e, Transport::Tcp).is_err());
        t.connect(slot, Endpoint::new(Ipv4Address::LOOPBACK, 443))
            .unwrap();
        assert!(t.get(slot).unwrap().connected);
    ...
    fn udp_and_tcp_lengths_fail_closed() {
        ...
        let mut tcp = [0u8; 20];
        tcp[0..2].copy_from_slice(&1u16.to_be_bytes());
        tcp[2..4].copy_from_slice(&2u16.to_be_bytes());
        tcp[12] = 5 << 4;
