...
impl Route {
    pub fn matches(self, address: Ipv4Address) -> bool {
        ...
        (u32::from_be_bytes(self.network.0) & mask)
            == (u32::from_be_bytes(address.0) & mask)
    }
}
    ...
    pub fn lookup(&self, address: Ipv4Address) -> Result<Route, NetError> {
        ...
        for route in self.routes.iter().flatten().copied() {
            if route.matches(address)
                && best.map_or(true, |current: Route| route.prefix_len > current.prefix_len)
            {
                best = Some(route);
            }
        }
...
mod tests {
    ...
    fn route_lookup_uses_longest_prefix() {
        ...
        let route = table
            .lookup(Ipv4Address::new([10, 1, 2, 3]))
            .expect("route lookup");
    ...
    fn route_table_rejects_invalid_prefix_and_capacity_overflow() {
        ...
        table.add(route).expect("first route");
