...
impl Rule {
    pub fn matches(self, endpoint: Endpoint, transport: Transport) -> bool {
        ...
        if let Some(expected) = self.transport
            && expected != transport
        {
            return false;
        }
        ...
        (u32::from_be_bytes(self.network.0) & mask)
            == (u32::from_be_bytes(endpoint.address.0) & mask)
    }
    pub const fn specificity(self) -> u16 {
        (self.prefix_len as u16) * 4
            + if self.port.is_some() { 2 } else { 0 }
            + if self.transport.is_some() { 1 } else { 0 }
    }
}
pub struct Firewall<const N: usize = MAX_RULES> {
    rules: [Option<Rule>; N],
}
impl<const N: usize> Default for Firewall<N> {
    fn default() -> Self {
        Self::new()
    }
}
impl<const N: usize> Firewall<N> {
    ...
    pub fn add(&mut self, rule: Rule) -> Result<usize, NetError> {
        if rule.prefix_len > 32 || rule.port == Some(0) {
...
mod tests {
    ...
    fn policy_is_deny_by_default_and_bounded() {
        ...
        assert_eq!(
            fw.decide(Endpoint::new(Ipv4Address::LOOPBACK, 80), Transport::Tcp),
            Action::Deny
        );
    }
    #[test]
    fn most_specific_rule_wins_independent_of_insertion_order() {
        let endpoint = Endpoint::new(Ipv4Address::new([10, 1, 2, 3]), 443);
        let mut fw = Firewall::<2>::new();
        fw.add(Rule {
    ...
    #[test]
    fn invalid_rule_and_capacity_fail_closed() {
        let mut fw = Firewall::<1>::new();
        assert_eq!(
            fw.add(Rule {
        ...
        fw.add(rule).unwrap();
        assert_eq!(fw.add(rule), Err(NetError::WouldBlock));
    }
}
