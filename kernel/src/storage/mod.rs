...
pub enum StorageError { InvalidBlock, BufferTooSmall, ReadOnly, Io, Unsupported, InvalidMetadata, TooLarge }
impl From<GptError> for StorageError {
    fn from(error: GptError) -> Self { match error {
        GptError::BufferTooSmall => Self::BufferTooSmall,
        GptError::InvalidHeaderSize | GptError::InvalidLbaRange | GptError::InvalidEntrySize |
        GptError::TooManyPartitions | GptError::InvalidPartitionRange | GptError::HeaderCrcMismatch |
        GptError::PartitionArrayCrcMismatch | GptError::BadSignature | GptError::UnsupportedRevision => Self::InvalidMetadata,
    }}
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct DeviceGeometry { pub block_size: u32, pub blocks: u64, pub read_only: bool }
impl DeviceGeometry {
    pub const fn new(block_size: u32, blocks: u64, read_only: bool) -> Self { Self { block_size, blocks, read_only } }
    pub const fn bytes(self) -> Option<u64> { self.blocks.checked_mul(self.block_size as u64) }
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct GptScanSummary { pub header: GptHeader, pub partitions: u16 }

pub fn scan_gpt<D: BlockDevice>(device: &mut D) -> Result<GptScanSummary, StorageError> {
    if device.block_size() < gpt::GPT_SECTOR_SIZE || !device.block_size().is_multiple_of(gpt::GPT_SECTOR_SIZE) { return Err(StorageError::Unsupported); }
    let sectors_per_block = device.block_size() / gpt::GPT_SECTOR_SIZE;
    let disk_last_lba = device.block_count().checked_mul(sectors_per_block as u64).and_then(|s| s.checked_sub(1)).ok_or(StorageError::InvalidBlock)?;
    let mut block = [0u8; BLOCK_SIZE];
    device.read_block(0, &mut block)?;
    let header = gpt::parse_header(&block[gpt::GPT_SECTOR_SIZE..], disk_last_lba)?;
    let entry_bytes = (header.partition_count as usize).checked_mul(header.partition_entry_size as usize).ok_or(StorageError::TooLarge)?;
    if entry_bytes > 16 * 1024 { return Err(StorageError::TooLarge); }
    let mut entries = [0u8; 16 * 1024];
    let first_block = header.partition_entry_lba / sectors_per_block as u64;
    let first_sector_in_block = (header.partition_entry_lba % sectors_per_block as u64) as usize * gpt::GPT_SECTOR_SIZE;
    ...
    for index in 0..block_count {
        ...
        let destination_start = if index == 0 { 0 } else { index * device.block_size() - first_sector_in_block };
        let copy_len = core::cmp::min(device.block_size() - source_start, entry_bytes.saturating_sub(destination_start));
        if copy_len == 0 { break; }
        entries[destination_start..destination_start + copy_len].copy_from_slice(&block[source_start..source_start + copy_len]);
    }
    ...
    for index in 0..header.partition_count as usize {
        ...
        if gpt::parse_partition(&entries[start..end], &header, disk_last_lba)?.is_some() { partitions = partitions.saturating_add(1); }
    }
...
mod tests {
    ...
    fn scans_real_gpt_metadata_through_block_device() {
        ...
        block[entry_offset] = 1; block[entry_offset + 16] = 2;
        ...
        block[header_offset + 8..header_offset + 12].copy_from_slice(&gpt::GPT_REVISION_1_0.to_le_bytes());
        block[header_offset + 12..header_offset + 16].copy_from_slice(&(gpt::GPT_HEADER_MIN_SIZE as u32).to_le_bytes());
        ...
        block[header_offset + 84..header_offset + 88].copy_from_slice(&(gpt::GPT_PARTITION_ENTRY_MIN_SIZE as u32).to_le_bytes());
        block[header_offset + 88..header_offset + 92].copy_from_slice(&partition_crc.to_le_bytes());
        let mut header_copy = block; header_copy[header_offset + 16..header_offset + 20].fill(0);
        ...
        assert_eq!(summary.partitions, 1); assert_eq!(summary.header.partition_count, 1);
    }
}
