...
impl<const N: usize> FileStore<N> {
    ...
    pub fn create(&mut self, name: &[u8]) -> Result<FileRecord, FileStoreError> {
        ...
        self.slots[slot].record = Some(record);
        self.slots[slot].name[..name.len()].copy_from_slice(name);
        self.slots[slot].name_len = name.len() as u8;
        Ok(record)
    }

    pub fn write_at<D: BlockDevice>(&mut self, device: &mut D, id: u32, offset: usize, data: &[u8]) -> Result<usize, FileStoreError> {
        let index = self.find_id(id).ok_or(FileStoreError::NotFound)?;
        let record = self.slots[index].record.unwrap();
        let end = offset.checked_add(data.len()).ok_or(FileStoreError::TooLarge)?;
        if end > MAX_FILE_SIZE { return Err(FileStoreError::TooLarge); }
        if data.is_empty() { return Ok(0); }
        let required_blocks = end.div_ceil(BLOCK_SIZE);
        if required_blocks > MAX_FILE_BLOCKS { return Err(FileStoreError::TooLarge); }
        if required_blocks > record.blocks as usize {
            let additional = required_blocks - record.blocks as usize;
            if self.next_block.checked_add(additional as u64).is_none() { return Err(FileStoreError::NoSpace); }
            self.slots[index].record.as_mut().unwrap().blocks = required_blocks as u16;
            self.next_block = self.next_block.saturating_add(additional as u64);
        }
        let first = record.first_block;
        let mut consumed = 0usize;
        while consumed < data.len() {
            let absolute = offset + consumed;
            let block = first + (absolute / BLOCK_SIZE) as u64;
            let in_block = absolute % BLOCK_SIZE;
            let take = core::cmp::min(BLOCK_SIZE - in_block, data.len() - consumed);
        ...
        let current = self.slots[index].record.as_mut().unwrap();
        if end as u64 > current.size { current.size = end as u64; }
    ...
    pub fn read_at<D: BlockDevice>(&self, device: &mut D, id: u32, offset: usize, out: &mut [u8]) -> Result<usize, FileStoreError> {
        let index = self.find_id(id).ok_or(FileStoreError::NotFound)?;
        let record = self.slots[index].record.unwrap();
        if offset > record.size as usize { return Err(FileStoreError::InvalidOffset); }
        let available = record.size as usize - offset;
        let count = core::cmp::min(available, out.len());
        let mut consumed = 0usize;
        while consumed < count {
            let absolute = offset + consumed;
            let block = record.first_block + (absolute / BLOCK_SIZE) as u64;
            let in_block = absolute % BLOCK_SIZE;
            let take = core::cmp::min(BLOCK_SIZE - in_block, count - consumed);
            let mut buf = [0u8; BLOCK_SIZE];
            device.read_block(block, &mut buf)?;
            out[consumed..consumed + take].copy_from_slice(&buf[in_block..in_block + take]);
            consumed += take;
        }
        Ok(count)
    }

    pub fn truncate(&mut self, id: u32, size: usize) -> Result<(), FileStoreError> {
        if size > MAX_FILE_SIZE { return Err(FileStoreError::TooLarge); }
        let index = self.find_id(id).ok_or(FileStoreError::NotFound)?;
        let r = self.slots[index].record.as_mut().unwrap();
        r.size = size as u64;
        r.blocks = size.div_ceil(BLOCK_SIZE) as u16;
        r.generation = r.generation.saturating_add(1);
        Ok(())
    }

    pub fn delete(&mut self, id: u32) -> Result<(), FileStoreError> {
        let index = self.find_id(id).ok_or(FileStoreError::NotFound)?;
...
mod tests {
    ...
    #[test]
    fn block_backed_round_trip_supports_seek() {
        let mut disk = RamBlockDevice::default();
        let mut fs = FileStore::<4>::new(8);
        let file = fs.create(b"persist.bin").unwrap();
        fs.write_at(&mut disk, file.id, 3, b"AWEOS").unwrap();
        let mut out = [0u8; 8];
        assert_eq!(fs.read_at(&mut disk, file.id, 3, &mut out).unwrap(), 5);
        assert_eq!(&out[..5], b"AWEOS");
        assert_eq!(fs.record(file.id).unwrap().size, 8);
    }
    #[test]
    fn rejects_out_of_bounds_file_size() {
        let mut disk = RamBlockDevice::default();
        let mut fs = FileStore::<1>::new(8);
        let file = fs.create(b"x").unwrap();
        assert_eq!(fs.write_at(&mut disk, file.id, MAX_FILE_SIZE, b"x"), Err(FileStoreError::TooLarge));
    }
}