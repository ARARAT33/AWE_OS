...
pub enum RecoveryError { Health(HealthError), Instance(MultiInstanceError), InvalidIndex, RecoveryLimit }
...
impl<const N: usize> FaultRecovery<N> {
    pub const fn new(max_attempts: u8) -> Self {
        Self { health: DriverHealthMonitor::new(), attempts: [0; N], max_attempts }
    }
    ...
    pub fn probe_result(&mut self, index: usize, success: bool) -> Result<(), RecoveryError> {
        self.health.report_probe(index, success).map_err(RecoveryError::Health)
    }

    pub fn recover<const M: usize>(&mut self, index: usize, manager: &mut MultiInstanceManager<M>) -> Result<(), RecoveryError> {
        if index >= self.health.count { return Err(RecoveryError::InvalidIndex); }
        if self.attempts[index] >= self.max_attempts { return Err(RecoveryError::RecoveryLimit); }
        if self.health.entries[index].state == HealthState::Quarantined { return Err(RecoveryError::Health(HealthError::Quarantined)); }
        self.attempts[index] = self.attempts[index].saturating_add(1);
        manager.rollback_instance(index).map_err(RecoveryError::Instance)?;
        ...
        self.health.request_recovery(index).map_err(RecoveryError::Health)?;
        Ok(())
    }

    pub fn quarantine<const M: usize>(&mut self, index: usize, manager: &mut MultiInstanceManager<M>) -> Result<(), RecoveryError> {
        if index >= self.health.count { return Err(RecoveryError::InvalidIndex); }
...
mod tests {
    ...
    fn bounded_recovery_restarts_instance() {
        ...
        m.add(100).unwrap(); m.activate_all().unwrap();
    ...
    fn recovery_limit_is_enforced() {
        ...
        let i = r.attach().unwrap();
        let mut m = MultiInstanceManager::<1>::new(); m.add(7).unwrap(); m.activate_all().unwrap();
        r.probe_result(i, false).unwrap(); r.recover(i, &mut m).unwrap();
