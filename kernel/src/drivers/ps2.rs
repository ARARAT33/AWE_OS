...
pub enum Ps2Error { Timeout, Controller, QueueFull, InvalidPacket, Unsupported }

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum KeyCode { Escape, Enter, Backspace, Tab, Space, Left, Right, Up, Down, Character(u8), Unknown(u8) }

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum Ps2Event { Key { code: KeyCode, pressed: bool }, Pointer { dx: i16, dy: i16, buttons: u8 } }

pub struct EventQueue { slots: [Option<Ps2Event>; MAX_EVENTS], head: usize, len: usize }
impl EventQueue {
    pub const fn new() -> Self { Self { slots: [None; MAX_EVENTS], head: 0, len: 0 } }
    ...
    pub fn push(&mut self, event: Ps2Event) -> Result<(), Ps2Error> {
        ...
        self.slots[index] = Some(event); self.len += 1; Ok(())
    }
    pub fn pop(&mut self) -> Option<Ps2Event> {
        if self.len == 0 { return None; }
        let event = self.slots[self.head].take();
        self.head = (self.head + 1) % MAX_EVENTS; self.len -= 1; event
    }
}
impl Default for EventQueue { fn default() -> Self { Self::new() } }

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct KeyboardDecoder { extended: bool, break_code: bool }
        ...
        match byte {
            ...
            code => {
                ...
                let mapped = if self.extended {
                    match code { 0x4B => KeyCode::Left, 0x4D => KeyCode::Right, 0x48 => KeyCode::Up, 0x50 => KeyCode::Down, other => KeyCode::Unknown(other) }
                } else {
                    match code { 0x01 => KeyCode::Escape, 0x0D => KeyCode::Tab, 0x1C => KeyCode::Enter, 0x0E => KeyCode::Backspace, 0x39 => KeyCode::Space, 0x10..=0x35 => KeyCode::Character(code), other => KeyCode::Unknown(other) }
                };
                self.extended = false; self.break_code = false;
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct MouseDecoder { packet: [u8; 3], index: usize }
impl MouseDecoder {
    pub const fn new() -> Self { Self { packet: [0; 3], index: 0 } }
    pub fn feed(&mut self, byte: u8) -> Result<Option<Ps2Event>, Ps2Error> {
        if self.index == 0 && byte & 0x08 == 0 { return Ok(None); }
        self.packet[self.index] = byte; self.index += 1;
        if self.index < 3 { return Ok(None); }
        self.index = 0;
        let flags = self.packet[0];
        if flags & 0xC0 != 0 { return Err(Ps2Error::InvalidPacket); }
        let x = self.packet[1] as i16; let y = self.packet[2] as i16;
        let dx = if flags & 0x10 != 0 { x - 256 } else { x };
        let dy_raw = if flags & 0x20 != 0 { y - 256 } else { y };
        Ok(Some(Ps2Event::Pointer { dx, dy: -dy_raw, buttons: flags & 0x07 }))
    }
}
impl Default for MouseDecoder { fn default() -> Self { Self::new() } }

pub struct Controller { pub events: EventQueue, pub keyboard: KeyboardDecoder, pub mouse: MouseDecoder, pub initialized: bool }
impl Controller {
    pub const fn new() -> Self { Self { events: EventQueue::new(), keyboard: KeyboardDecoder::new(), mouse: MouseDecoder::new(), initialized: false } }
    ...
    pub unsafe fn init(&mut self) -> Result<(), Ps2Error> {
        ...
        unsafe { io_out8(COMMAND_PORT, 0xAE); }
        self.initialized = true; Ok(())
    }

    #[cfg(not(target_arch = "x86_64"))]
    pub unsafe fn init(&mut self) -> Result<(), Ps2Error> { Err(Ps2Error::Unsupported) }
    ...
    pub unsafe fn poll(&mut self, max_bytes: usize) -> Result<usize, Ps2Error> {
        ...
        while read < max_bytes {
            ...
            } else if let Some(event) = self.keyboard.feed(byte) { self.events.push(event)?; }
            read += 1;
        }
        Ok(read)
    }
    ...
    #[cfg(not(target_arch = "x86_64"))]
    pub unsafe fn poll(&mut self, _max_bytes: usize) -> Result<usize, Ps2Error> { Err(Ps2Error::Unsupported) }
}
...
mod tests {
    use super::*;
    #[test]
    fn keyboard_decodes_press_release_and_extended() {
    ...
    #[test]
    fn mouse_decodes_signed_motion() {
