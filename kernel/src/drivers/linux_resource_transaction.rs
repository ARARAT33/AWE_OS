...
pub enum ResourceTransactionError { Resource(ResourceError), Capacity, NotActive }
...
impl<'a, const N: usize> ResourceTransaction<'a, N> {
    pub fn begin(manager: &'a mut ResourceManager<N>) -> Self {
        Self { manager, acquired: [None; N], count: 0, active: true }
    }

    pub fn acquire(&mut self, resource: Resource) -> Result<usize, ResourceTransactionError> {
        if !self.active { return Err(ResourceTransactionError::NotActive); }
        if self.count == N { return Err(ResourceTransactionError::Capacity); }
        let slot = self.manager.acquire(resource).map_err(ResourceTransactionError::Resource)?;
    ...
    pub fn rollback(&mut self) -> Result<usize, ResourceTransactionError> {
        ...
        while self.count != 0 {
            ...
            if let Some((slot, owner)) = self.acquired[self.count].take() {
                self.manager.release(owner, slot).map_err(ResourceTransactionError::Resource)?;
                released += 1;
            }
        }
        self.active = false;
...
mod tests {
    ...
    fn failed_transaction_rolls_back_all_resources() {
        let mut manager = ResourceManager::<4>::new();
        {
            ...
            tx.acquire(Resource { owner: 1, kind: ResourceKind::Mmio, start: 0x1000, length: 0x100 }).unwrap();
            tx.acquire(Resource { owner: 1, kind: ResourceKind::Irq, start: 5, length: 1 }).unwrap();
    ...
    fn drop_is_a_safety_rollback() {
        ...
        {
            let mut tx = ResourceTransaction::begin(&mut manager);
            tx.acquire(Resource { owner: 4, kind: ResourceKind::Dma, start: 10, length: 2 }).unwrap();
        }
        assert_eq!(manager.count(), 0);
    }
    ...
    fn commit_keeps_resources() {
        let mut manager = ResourceManager::<2>::new();
        let mut tx = ResourceTransaction::begin(&mut manager);
        tx.acquire(Resource { owner: 9, kind: ResourceKind::Mmio, start: 0x2000, length: 0x20 }).unwrap();
        tx.commit().unwrap();
