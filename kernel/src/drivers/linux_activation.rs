...
pub enum ActivationError { Order(OrderError), OutputFull, InvalidSequence }
...
pub fn validate_activation_order(
    ...
    for dep in deps {
        let driver_pos = order.iter().position(|n| *n == dep.driver_hash)
            .ok_or(ActivationError::InvalidSequence)?;
        let required_pos = order.iter().position(|n| *n == dep.required_hash)
            .ok_or(ActivationError::InvalidSequence)?;
...
mod tests {
    ...
    fn builds_dependency_first_order() {
        let nodes = [1, 2, 3];
        let deps = [
            Dependency { driver_hash: 1, required_hash: 2 },
            Dependency { driver_hash: 2, required_hash: 3 },
        ];
        let mut order = [0; 3];
    ...
    fn rejects_wrong_order() {
        ...
        assert_eq!(validate_activation_order(&[1, 2], &deps), Err(ActivationError::InvalidSequence));
