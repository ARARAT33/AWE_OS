...
use super::linux_package::{LDRIVER_MAGIC, LinuxPackageHeader};
use super::linux_resolver::LinuxCandidate;
use super::linux_runtime::{LinuxDriverDescriptor, LinuxRuntime};
use super::linux_transaction::{DriverTransaction, TransactionError, TransactionState};
use super::linux_transaction_orchestrator::{
    ActivationOrchestrator, OrchestratorError, OrchestratorState,
};
...
impl<const N: usize> TransactionBridge<N> {
    pub const fn new() -> Self {
        Self {
            transaction: None,
            orchestrator: ActivationOrchestrator::new(),
    ...
    pub fn prepare(&mut self, nodes: &[u64], deps: &[Dependency]) -> Result<(), BridgeError> {
        let id = DeviceId {
            vendor: 0x8086,
            device: (if nodes.is_empty() { 1 } else { nodes[0] as u16 }).max(1),
            class: 0,
            revision: 1,
        };
        let desc = LinuxDriverDescriptor {
            vendor: id.vendor,
            device: id.device,
            class: 0,
            api_version: 1,
        ...
        let tx =
            DriverTransaction::prepare(id, &[candidate], &LinuxRuntime::new(1), header, resources)
                .map_err(BridgeError::Transaction)?;
        self.transaction = Some(tx);
        self.orchestrator
            .prepare(nodes, deps)
            .map_err(BridgeError::Orchestrator)?;
        self.lifecycle
            .apply(DriverOp::Probe, true)
            .map_err(BridgeError::Lifecycle)?;
        let t = self
            .transaction
            .as_ref()
            .ok_or(BridgeError::MissingTransaction)?;
        if t.state != TransactionState::Prepared
            || self.orchestrator.state != OrchestratorState::Ready
            || self.lifecycle.state != DriverState::Probed
        {
    ...
    pub fn begin(&mut self) -> Result<(), BridgeError> {
        ...
        let state = self
            ...
            .ok_or(BridgeError::MissingTransaction)?
            .state;
        if state != TransactionState::Activating
            || self.orchestrator.state != OrchestratorState::Activating
            || self.lifecycle.state != DriverState::Initialized
        {
            return Err(BridgeError::StateMismatch);
        }
        Ok(())
    ...
    pub fn mark_activated(&mut self) -> Result<u64, BridgeError> {
        ...
        let tx = self
            ...
            .map_err(BridgeError::Transaction)?;
        self.transaction = Some(tx);
        if self.orchestrator.state == OrchestratorState::Activating
            && self.lifecycle.state == DriverState::Initialized
        {
            self.lifecycle
        ...
        if self.orchestrator.state == OrchestratorState::Activated
            && (state != TransactionState::Activated
                || self.lifecycle.state != DriverState::Running)
        {
            return Err(BridgeError::StateMismatch);
        }
        Ok(node)
    ...
    pub fn complete_rollback(&mut self) -> Result<(), BridgeError> {
        ...
        self.orchestrator
            .complete_rollback()
            .map_err(BridgeError::Orchestrator)?;
        if self.lifecycle.state == DriverState::Running {
            self.lifecycle
                .apply(DriverOp::Stop, true)
                .map_err(BridgeError::Lifecycle)?
        }
        if self.lifecycle.state == DriverState::Stopped {
            self.lifecycle
        ...
        if state != TransactionState::RolledBack
            || self.orchestrator.state != OrchestratorState::RolledBack
            || self.lifecycle.state != DriverState::Removed
        {
            return Err(BridgeError::StateMismatch);
        }
        Ok(())
