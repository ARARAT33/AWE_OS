...
impl<const M: usize> DeviceContract<M> {
    pub const fn valid(&self) -> bool {
        self.vendor != 0 && self.device != 0 && self.dma.max_bytes != 0 && self.dma.address_bits >= 32
    }

    pub const fn allows_mmio(&self, address: u64) -> bool {
        let mut i = 0;
        while i < M {
            if let Some(region) = self.mmio[i] {
...
mod tests {
    ...
    #[test]
    fn mmio_range_is_overflow_safe() {
        let r = MmioRegion { base: 0x1000, length: 0x100 };
        assert!(r.contains(0x1080));
        assert!(!r.contains(0x1100));
        assert!(!MmioRegion { base: u64::MAX, length: 2 }.contains(u64::MAX));
    }
}
