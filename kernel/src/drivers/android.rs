...
mod tests{
 ...
 fn good()->AndroidLayer<Mock>{AndroidLayer::new(Mock{api:7,name:"awe-hal",os:DriverOs::Android,abi:DriverAbi::AndroidHal,vendor:1,device:2,signed:true})}
 #[test]fn valid_adapter_passes(){assert!(good().validate(&hw()).is_ok())}
 #[test]fn mismatched_identity_is_rejected(){let mut x=good();x.slot.adapter.device=9;assert_eq!(x.validate(&hw()),Err(CoreError::InvalidDevice))}
