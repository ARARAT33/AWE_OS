...
impl<A: LinuxDriverAdapter> LinuxLayer<A> {
    ...
    pub fn validate(&self,hw:&HardwareInfo)->Result<(),CoreError>{
        ...
        if !hw.valid()||hw.irq==u32::MAX{return Err(CoreError::InvalidDevice)}
        Ok(())
    }
}
...
mod tests{
 ...
 struct Mock{api:u32,name:&'static str,vendor:u16,device:u16,signed:bool}
 impl DriverAdapter for Mock{fn identity(&self)->super::super::core::DriverIdentity{super::super::core::DriverIdentity{os:DriverOs::Linux,abi:DriverAbi::LinuxKmod,api_version:self.api as u16,vendor:self.vendor,device:self.device,signed:self.signed}}fn probe(&mut self,_:&HardwareInfo)->Result<(),CoreError>{Ok(())}fn start(&mut self,_:&HardwareInfo)->Result<(),CoreError>{Ok(())}fn stop(&mut self,_:&HardwareInfo)->Result<(),CoreError>{Ok(())}fn remove(&mut self,_:&HardwareInfo)->Result<(),CoreError>{Ok(())}}
 impl LinuxDriverAdapter for Mock{fn linux_api_version(&self)->u32{self.api}fn linux_module_name(&self)->&'static str{self.name}}
 ...
 fn good()->LinuxLayer<Mock>{LinuxLayer::new(Mock{api:6,name:"awe-net",vendor:1,device:2,signed:true})}
 #[test]fn valid_adapter_passes(){assert!(good().validate(&hw()).is_ok())}
 #[test]fn api_mismatch_is_rejected(){let mut x=good();x.slot.adapter.api=7;assert_eq!(x.validate(&hw()),Err(CoreError::InvalidRequest))}
 #[test]fn invalid_irq_is_rejected(){let x=good();let mut h=hw();h.irq=u32::MAX;assert_eq!(x.validate(&h),Err(CoreError::InvalidDevice))}
