...
impl<const N: usize> DriverResourceBinding<N> {
    ...
    pub fn acquire_all(&mut self, requested: &[Resource]) -> Result<(), BindingError> {
        if self.active || self.has_resources() { return Err(BindingError::InvalidState); }
        ...
        tx.commit().map_err(BindingError::Transaction)?; self.active = true; Ok(())
    }
    pub fn probe_and_init(&mut self, driver: &mut DriverLifecycle) -> Result<(), BindingError> {
        if !self.active || driver.state != DriverState::New { return Err(BindingError::InvalidState); }
        driver.apply(DriverOp::Probe, true).map_err(BindingError::Lifecycle)?;
        if let Err(error) = driver.apply(DriverOp::Init, true) { let _ = driver.apply(DriverOp::Remove, true); self.release_all(); return Err(BindingError::Lifecycle(error)); }
        Ok(())
    }
    pub fn start(&mut self, driver: &mut DriverLifecycle) -> Result<(), BindingError> {
        ...
        if let Err(error) = driver.apply(DriverOp::Start, true) { let _ = driver.apply(DriverOp::Remove, true); self.release_all(); return Err(BindingError::Lifecycle(error)); }
        Ok(())
    }
    pub fn stop(&mut self, driver: &mut DriverLifecycle) -> Result<(), BindingError> {
        ...
        if matches!(driver.state, DriverState::Stopped | DriverState::Initialized | DriverState::Probed) { driver.apply(DriverOp::Remove, true).map_err(BindingError::Lifecycle)?; }
        self.release_all(); Ok(())
    }
