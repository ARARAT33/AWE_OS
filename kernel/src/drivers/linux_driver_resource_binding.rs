...
impl<const N: usize> DriverResourceBinding<N> {
    pub const fn new(owner: u64) -> Self {
        Self { owner, resources: ResourceManager::new(), active: false }
    }
    ...
    pub fn stop(&mut self, driver: &mut DriverLifecycle) -> Result<(), BindingError> {
        if driver.state == DriverState::Running {
            driver.apply(DriverOp::Stop, true).map_err(BindingError::Lifecycle)?;
        }
        self.release_all();
        Ok(())
    }
...
mod tests {
    ...
    fn failed_second_resource_rolls_back_first() {
        ...
        let requested = [
            Resource { owner: 10, kind: ResourceKind::Mmio, start: 0x1000, length: 0x100 },
            Resource { owner: 10, kind: ResourceKind::Mmio, start: 0x1080, length: 0x20 },
        ];
