...
impl<const N: usize> DriverSupervisor<N> {
    ...
    pub fn register(
        ...
        let index = self
            .registry
            .register(id, class, vendor, device)
    ...
    pub fn apply(
        ...
        self.health
            .report_probe(health_index, success)
    ...
    pub fn recover(&mut self, id: u64) -> Result<DriverState, SupervisorError> {
        ...
        match record.lifecycle {
            DriverState::Empty => Err(SupervisorError::RecoveryUnavailable),
            DriverState::New => {
                self.registry
                    .apply(id, DriverOp::Probe, true)
                    .map_err(SupervisorError::Registry)?;
                self.registry
                    .apply(id, DriverOp::Init, true)
                    .map_err(SupervisorError::Registry)?;
                self.registry
                    .apply(id, DriverOp::Start, true)
                    .map_err(SupervisorError::Registry)
            }
            DriverState::Probed => {
                self.registry
            ...
            DriverState::Initialized => self
                .registry
                .apply(id, DriverOp::Start, true)
                .map_err(SupervisorError::Registry),
            DriverState::Running => Ok(DriverState::Running),
            DriverState::Stopped | DriverState::Removed => {
    ...
    fn registry_index(&self, id: u64) -> Result<usize, RegistryError> {
        ...
        while i < self.registry.len() {
            if self.registry.get_at(i)?.id == id {
                return Ok(i);
            }
            i += 1
        }
