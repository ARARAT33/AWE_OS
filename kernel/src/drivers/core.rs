...
use super::contract::{DeviceContract, InterruptMode};
...
impl HardwareInfo {
    pub const fn valid(&self) -> bool {
        self.id.vendor != 0
            && self.id.vendor != 0xffff
            && self.id.device != 0
            && self.id.device != 0xffff
            && self.mmio_length != 0
            && self.dma_bits >= 32
            && self.dma_bits <= 64
            && self.mmio_base.checked_add(self.mmio_length).is_some()
    }

    pub const fn mmio_contains(&self, offset: u64, width: u64) -> bool {
        width != 0 && offset <= self.mmio_length && width <= self.mmio_length - offset
    }
}
...
impl DriverIdentity {
    pub const fn matches(&self, hw: &HardwareInfo) -> bool {
        self.vendor == hw.id.vendor
            && self.device == hw.id.device
            && self.signed
            && self.api_version != 0
    }
}
...
impl<A: DriverAdapter> DriverSlot<A> {
    ...
    pub fn dispatch<H: HardwareAbstraction>(
        ...
        match action {
            ...
            DriverAction::Stop => {
                ...
                self.adapter.stop(hw).map_err(|_| CoreError::NotBound)?;
            ...
            DriverAction::Remove => {
                ...
                self.adapter.remove(hw).map_err(|_| CoreError::NotBound)?;
    ...
    pub fn mmio_read32(
        &self,
        hw: &HardwareInfo,
        hal: &impl HardwareAbstraction,
        offset: u64,
    ) -> Result<u32, CoreError> {
        if self.state != AdapterState::Running {
            return Err(CoreError::NotBound);
        }
        if !hw.mmio_contains(offset, 4) {
            return Err(CoreError::MmioDenied);
        }
        hal.mmio_read32(hw, offset)
    }

    pub fn mmio_write32(
        &self,
        hw: &HardwareInfo,
        hal: &mut impl HardwareAbstraction,
        offset: u64,
        value: u32,
    ) -> Result<(), CoreError> {
        if self.state != AdapterState::Running {
        ...
        hal.mmio_write32(hw, offset, value)
    }

    pub fn mmio_read32_contract<const M: usize>(
        &self,
        hw: &HardwareInfo,
        ...
        if self.state != AdapterState::Running
            || !contract.valid()
            || contract.vendor != hw.id.vendor
            || contract.device != hw.id.device
            || !contract.allows_mmio_range(offset, 4)
        {
            return Err(CoreError::MmioDenied);
        }
        hal.mmio_read32(hw, offset)
    ...
    pub fn mmio_write32_contract<const M: usize>(
        &self,
        hw: &HardwareInfo,
        contract: &DeviceContract<M>,
        hal: &mut impl HardwareAbstraction,
    ...
    pub fn mmio_read32_contract_base<const M: usize>(
        &self,
        hw: &HardwareInfo,
        contract: &DeviceContract<M>,
        hal: &impl HardwareAbstraction,
        ...
        if self.state != AdapterState::Running
            ...
            || contract.vendor != hw.id.vendor
            || contract.device != hw.id.device
            || !contract.allows_mmio_range(absolute, 4)
        {
            return Err(CoreError::MmioDenied);
        }
        hal.mmio_read32(hw, offset)
    ...
    pub fn mmio_write32_contract_base<const M: usize>(
        &self,
        hw: &HardwareInfo,
        contract: &DeviceContract<M>,
        hal: &mut impl HardwareAbstraction,
    ...
    pub fn dma_submit_contract<const M: usize>(
        &self,
        hw: &HardwareInfo,
        contract: &DeviceContract<M>,
        hal: &mut impl HardwareAbstraction,
        ...
        if !contract.allows_dma(bytes, address_bits) {
            return Err(CoreError::DmaDenied);
        }
        hal.dma_submit(hw, bytes)
    }

    pub fn dma_submit_contract_range<const M: usize>(
        &self,
        hw: &HardwareInfo,
        contract: &DeviceContract<M>,
        hal: &mut impl HardwareAbstraction,
        ...
        if self.state != AdapterState::Running
            ...
            || contract.vendor != hw.id.vendor
            || contract.device != hw.id.device
            || !contract.allows_dma_range(address, bytes)
        {
            return Err(CoreError::DmaDenied);
        }
        hal.dma_submit(hw, bytes)
    ...
    pub fn irq_ack_contract<const M: usize>(
        &self,
        hw: &HardwareInfo,
        contract: &DeviceContract<M>,
        mode: InterruptMode,
        ...
        if self.state != AdapterState::Running
            ...
            || contract.vendor != hw.id.vendor
            || contract.device != hw.id.device
            || !contract.allows_interrupt(mode)
        {
            return Err(CoreError::IrqDenied);
        }
        hal.irq_ack(hw)
