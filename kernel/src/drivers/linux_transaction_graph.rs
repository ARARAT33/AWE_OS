...
/// Validates both direct dependencies and the complete bounded dependency
/// graph before a Linux driver transaction may be prepared.
pub fn prepare_graph_guarded(
    ...
    validate_graph(&nodes[..count], dependencies)
...
mod tests {
    ...
    fn candidate(hash: u64) -> LinuxCandidate {
        LinuxCandidate { descriptor: LinuxDriverDescriptor { vendor: 0x8086, device: 0x100e, class: 0x0200, api_version: 6, module_hash: hash, signed: true }, priority: 10 }
    }
    ...
    fn accepts_dependency_chain() {
        ...
        assert!(prepare_graph_guarded(DEV, &candidates, &deps, &LinuxRuntime::new(6), HDR, RES).is_ok());
    }

    #[test]
    fn rejects_cycle_before_prepare() {
        let candidates = [candidate(1), candidate(2), candidate(3)];
        let deps = [Dependency { driver_hash: 1, required_hash: 2 }, Dependency { driver_hash: 2, required_hash: 3 }, Dependency { driver_hash: 3, required_hash: 1 }];
        assert_eq!(prepare_graph_guarded(DEV, &candidates, &deps, &LinuxRuntime::new(6), HDR, RES), Err(GraphTransactionError::Graph(GraphError::Cycle)));
    }
}
