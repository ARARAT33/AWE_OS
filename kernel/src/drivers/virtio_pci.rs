...
impl VirtioPciTransport {
    ...
    pub fn negotiate(&mut self, driver_features: VirtioFeatures) -> Result<VirtioFeatures, PciError> {
        self.transport.acknowledge().map_err(|_| PciError::UnsupportedTransport)?;
        self.transport.set_driver().map_err(|_| PciError::UnsupportedTransport)?;
        self.transport
...
mod tests {
    ...
    const fn caps() -> VirtioPciCapabilities {
        VirtioPciCapabilities {
            common_cfg: Bar { base: 0x1000, size: 0x1000, is_io: false, prefetchable: false },
            notify_cfg: Bar { base: 0x2000, size: 0x1000, is_io: false, prefetchable: false },
            device_cfg: None,
            is_modern: true,
        }
    }
    ...
    fn rejects_non_virtio_device() {
        let id = PciId { vendor: 0x1234, ..virtio_id() };
        assert_eq!(
    ...
    fn validates_bar_alignment_and_overflow() {
        assert!(Bar { base: 0x1000, size: 0x1000, is_io: false, prefetchable: false }.validate().is_ok());
        assert_eq!(
            Bar { base: 0x1800, size: 0x1000, is_io: false, prefetchable: false }.validate(),
            Err(PciError::InvalidBar)
        );
        assert_eq!(
            Bar { base: u64::MAX - 3, size: 8, is_io: false, prefetchable: false }.end(),
    ...
    fn modern_virtio_reaches_driver_ready() {
        ...
        let negotiated = t.negotiate(VirtioFeatures::VERSION_1 | VirtioFeatures::RING_INDIRECT_DESC).unwrap();
    ...
    fn queue_limit_is_hard_bounded() {
        let mut t = VirtioPciTransport::new(virtio_id(), caps(), VirtioFeatures::VERSION_1, 4).unwrap();
