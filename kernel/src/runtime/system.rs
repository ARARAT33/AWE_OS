...
use awe_storaged::StorageDaemon;
...
impl SystemRuntime {
    ...
    fn register_process(&mut self,pid:u64,entry:u64,memory_pages:u32,ipc_messages:u64)->Result<(),()>{if pid==0||entry==0||self.processes.len()>=MAX_RUNTIME_PROCESSES{return Err(())}let slot=self.processes.len();let stack_ptr=unsafe{core::ptr::addr_of_mut!(PROCESS_STACKS[slot]) as *mut u8};let stack_top=stack_ptr as u64+16384;let d=ProcessDescriptor{id:ProcessId(pid),state:ProcessState::Created,budget:ResourceBudget{cpu_ticks:10_000,memory_bytes:memory_pages as u64*4096,ipc_messages}};let c=ProcessContext::new(ProcessId(pid),CpuContext::kernel_entry(entry,stack_top,0));self.processes.register(d,c).map_err(|_|())?;self.processes.make_runnable(ProcessId(pid)).map_err(|_|())}
    pub fn attach_framebuffer(&mut self,fb:FramebufferInfo)->Result<(),super::EndUserRuntimeError>{self.core.attach_framebuffer(fb)}
    pub fn mount_core_namespaces(&mut self,first_block:u64)->Result<(),crate::storage::NamespaceError>{let s=128;self.namespaces.mount(Namespace::Config,first_block)?;self.namespaces.mount(Namespace::Home,first_block+s)?;self.namespaces.mount(Namespace::Apps,first_block+s*2)?;self.namespaces.mount(Namespace::System,first_block+s*3)?;self.namespaces.mount(Namespace::Log,first_block+s*4)?;let v=self.storage.register_volume(awe_storaged::VolumeType::AweFsVolume,8192,first_block,false).map_err(|_|crate::storage::NamespaceError::Capacity)?;self.storage.mount_volume(v,0xAWE0_0001).map_err(|_|crate::storage::NamespaceError::Capacity)?;self.storage.create_snapshot(v,0).map_err(|_|crate::storage::NamespaceError::Capacity)?;self.persist_storage_state().map_err(|_|crate::storage::NamespaceError::Capacity)?;Ok(())}
    pub fn persist_storage_state(&mut self)->Result<usize,awe_storaged::persistence::PersistError>{let len=awe_storaged::persistence::export_state(&self.storage,&mut self.storage_state)?;self.storage_state_len=len;Ok(len)}
    pub fn restore_storage_state(&mut self)->Result<(),awe_storaged::persistence::PersistError>{if self.storage_state_len==0{return Ok(())}awe_storaged::persistence::import_state(&mut self.storage,&self.storage_state[..self.storage_state_len])}
    ...
    pub fn start_core_apps(&mut self)->Result<(),awe_appd::AppRuntimeError>{let e=entry_address(runtime_app_entry)as u64;for id in 1..=4{let pid=self.apps.start(AppId(id),ALL.0)?;if self.register_process(pid,e,32,64).is_err(){return Err(awe_appd::AppRuntimeError::SpawnFailed)}}Ok(())}
    pub fn route_ps2(&mut self,event:Ps2Event)->Result<RuntimeEvent,super::EndUserRuntimeError>{let t=match event{Ps2Event::Key{code,pressed}=>InputEvent::Key{code:key_code_value(code),pressed},Ps2Event::Pointer{dx,dy,buttons}=>{self.cursor_x=self.cursor_x.saturating_add(dx as i32);self.cursor_y=self.cursor_y.saturating_add(dy as i32);InputEvent::Pointer{x:self.cursor_x,y:self.cursor_y,buttons}}};self.core.push_input(t)?;self.windows.handle_input(t);Ok(RuntimeEvent::Input(t))}
