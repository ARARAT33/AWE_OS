...
fn spawn_service(entry: usize, service: ServiceId, memory_pages: u32, cpu_budget: u32) -> Result<u64, ()> {
    if entry == 0 || service.0 == 0 || memory_pages == 0 || cpu_budget == 0 { return Err(()); }
    Ok(service.0 as u64)
}
...
impl SystemRuntime {
    pub const fn new() -> Self {
        Self { core: EndUserRuntime::new(), windows: WindowManager::new(), services: Supervisor::new(spawn_service), apps: AppSupervisor::new(spawn_app, create_window), processes: ProcessManager::new(), namespaces: NamespaceManager::new(), network: NetworkDaemon::new(), cursor_x: 0, cursor_y: 0 }
    }
    ...
    pub fn register_core_services(&mut self) -> Result<(), awe_initd::RuntimeError> {
        ...
        let specs = [
            ServiceRuntimeSpec { spec: ServiceSpec { id: ServiceId(1), restart: RestartPolicy::Always, capability_mask: CapabilitySet::DEVICE.union(CapabilitySet::IPC).0, memory_limit_pages: 64, cpu_budget_ticks: 10_000 }, dependencies: n, dependency_count: 0, entry: runtime_service_entry as usize },
            ServiceRuntimeSpec { spec: ServiceSpec { id: ServiceId(2), restart: RestartPolicy::Always, capability_mask: CapabilitySet::STORAGE.union(CapabilitySet::IPC).0, memory_limit_pages: 64, cpu_budget_ticks: 10_000 }, dependencies: d1, dependency_count: 1, entry: runtime_service_entry as usize },
            ServiceRuntimeSpec { spec: ServiceSpec { id: ServiceId(3), restart: RestartPolicy::Always, capability_mask: CapabilitySet::NETWORK.union(CapabilitySet::IPC).0, memory_limit_pages: 64, cpu_budget_ticks: 10_000 }, dependencies: d1, dependency_count: 1, entry: runtime_service_entry as usize },
            ServiceRuntimeSpec { spec: ServiceSpec { id: ServiceId(4), restart: RestartPolicy::Always, capability_mask: CapabilitySet::IPC.0, memory_limit_pages: 32, cpu_budget_ticks: 5_000 }, dependencies: d1, dependency_count: 1, entry: runtime_service_entry as usize },
            ServiceRuntimeSpec { spec: ServiceSpec { id: ServiceId(5), restart: RestartPolicy::Always, capability_mask: CapabilitySet::PROCESS.union(CapabilitySet::IPC).0, memory_limit_pages: 64, cpu_budget_ticks: 10_000 }, dependencies: d4, dependency_count: 1, entry: runtime_service_entry as usize },
            ServiceRuntimeSpec { spec: ServiceSpec { id: ServiceId(6), restart: RestartPolicy::Always, capability_mask: CapabilitySet::UI.union(CapabilitySet::IPC).0, memory_limit_pages: 128, cpu_budget_ticks: 20_000 }, dependencies: d4, dependency_count: 1, entry: runtime_service_entry as usize },
        ];
    ...
    pub fn start_core_services(&mut self) -> Result<(), awe_initd::RuntimeError> {
        for id in 1..=6 {
            let pid = self.services.start(ServiceId(id))?;
            if self.register_process(pid, runtime_service_entry as usize as u64, 64, 128).is_err() { return Err(awe_initd::RuntimeError::SpawnFailed); }
        }
        Ok(())
    }
    ...
    pub fn start_core_apps(&mut self) -> Result<(), awe_appd::AppRuntimeError> {
        for id in 1..=4 {
            let pid = self.apps.start(AppId(id), ALL.0)?;
            if self.register_process(pid, runtime_app_entry as usize as u64, 32, 64).is_err() { return Err(awe_appd::AppRuntimeError::SpawnFailed); }
        }
        Ok(())
    }
