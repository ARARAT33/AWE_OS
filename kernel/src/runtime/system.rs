...
impl SystemRuntime {
    ...
    pub fn mount_core_namespaces(&mut self, first_block: u64) -> Result<(), crate::storage::NamespaceError> {
        ...
        let volume = self.storage.register_volume(awe_storaged::VolumeType::AweFsVolume, 8192, first_block, false).map_err(|_| crate::storage::NamespaceError::Capacity)?;
        self.storage.mount_volume(volume, 0xAWE0_0001).map_err(|_| crate::storage::NamespaceError::Capacity)?;
        self.storage.create_snapshot(volume, 0).map_err(|_| crate::storage::NamespaceError::Capacity)?;
        self.persist_storage_state().map_err(|_| crate::storage::NamespaceError::Capacity)?;
        Ok(())
    }
    ...
    pub fn register_core_services(&mut self) -> Result<(), awe_initd::RuntimeError> {
        let n = [None; awe_initd::runtime::MAX_DEPENDENCIES]; let d1 = [Some(ServiceId(1)), None, None, None, None, None, None, None]; let d4 = [Some(ServiceId(4)), None, None, None, None, None, None, None]; let entry = entry_address(runtime_service_entry);
        let specs = [
        ...
        for spec in specs { self.services.register(spec)?; } Ok(())
    }
    pub fn start_core_services(&mut self) -> Result<(), awe_initd::RuntimeError> { let entry=entry_address(runtime_service_entry) as u64; for id in 1..=6 { let pid=self.services.start(ServiceId(id))?; if self.register_process(pid,entry,64,128).is_err(){return Err(awe_initd::RuntimeError::SpawnFailed);} } Ok(()) }
    pub fn admit_core_apps(&mut self) -> Result<(), awe_appd::AppRuntimeError> { let apps=[(1u64,CapabilitySet::UI.union(CapabilitySet::IPC).union(CapabilitySet::STORAGE)),(2u64,CapabilitySet::UI.union(CapabilitySet::IPC).union(CapabilitySet::STORAGE)),(3u64,CapabilitySet::UI.union(CapabilitySet::IPC).union(CapabilitySet::STORAGE)),(4u64,CapabilitySet::UI.union(CapabilitySet::IPC))]; for(id,caps)in apps{let manifest=AppManifest{id:AppId(id),abi_major:awe_appd::AWE_APP_ABI_MAJOR,abi_minor:awe_appd::AWE_APP_ABI_MINOR,memory_limit_pages:32,capability_mask:caps.0,dependency_count:0,resource_count:0};self.apps.admit(manifest).map_err(|_|awe_appd::AppRuntimeError::InvalidManifest)?;}Ok(()) }
    pub fn start_core_apps(&mut self) -> Result<(), awe_appd::AppRuntimeError> { let entry=entry_address(runtime_app_entry) as u64; for id in 1..=4{let pid=self.apps.start(AppId(id),ALL.0)?;if self.register_process(pid,entry,32,64).is_err(){return Err(awe_appd::AppRuntimeError::SpawnFailed);}}Ok(()) }
    pub fn route_ps2(&mut self,event:Ps2Event)->Result<RuntimeEvent,super::EndUserRuntimeError>{let translated=match event{Ps2Event::Key{code,pressed}=>InputEvent::Key{code:key_code_value(code),pressed},Ps2Event::Pointer{dx,dy,buttons}=>{self.cursor_x=self.cursor_x.saturating_add(dx as i32);self.cursor_y=self.cursor_y.saturating_add(dy as i32);InputEvent::Pointer{x:self.cursor_x,y:self.cursor_y,buttons}}};self.core.push_input(translated)?;self.windows.handle_input(translated);Ok(RuntimeEvent::Input(translated))}
