...
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub struct TimerScheduler { quantum: Quantum, pending: bool }

impl TimerScheduler {
    pub const fn new(quantum_ticks: u32) -> Self { Self { quantum: Quantum::new(quantum_ticks), pending: false } }
    ...
    pub fn on_tick(&mut self) -> TickDecision {
        if self.quantum.consume_tick() { self.pending = true; TickDecision::RequestReschedule } else { TickDecision::Continue }
    }
    pub fn take_reschedule(&mut self) -> bool { let was_pending = self.pending; self.pending = false; was_pending }
...
pub fn interrupt_tick() -> TickDecision {
    ...
    if previous == 0 { TickDecision::Continue } else { TickDecision::RequestReschedule }
}

pub fn interrupt_ticks() -> u64 { TIMER_TICKS.load(Ordering::Acquire) }
pub fn pending_interrupt_requests() -> u64 { RESCHEDULE_REQUESTS.load(Ordering::Acquire) }
pub fn consume_interrupt_request() -> bool {
    let mut current = RESCHEDULE_REQUESTS.load(Ordering::Acquire);
    loop {
        if current == 0 { return false; }
        match RESCHEDULE_REQUESTS.compare_exchange_weak(current, current - 1, Ordering::AcqRel, Ordering::Acquire) {
            Ok(_) => return true,
            Err(observed) => current = observed,
        }
    }
...
mod tests {
    use super::*;
    #[test] fn ticks_are_monotonic() { let timer = Timer::new(1000); assert_eq!(timer.tick(), 1); assert_eq!(timer.tick(), 2); assert_eq!(timer.elapsed_ms(), 2); }
    #[test] fn timer_scheduler_requests_then_consumes() { let mut s = TimerScheduler::new(2); assert_eq!(s.on_tick(), TickDecision::Continue); assert_eq!(s.on_tick(), TickDecision::RequestReschedule); assert!(s.pending()); assert!(s.take_reschedule()); assert!(!s.pending()); }
    #[test] fn zero_quantum_requests_safely() { let mut s = TimerScheduler::new(0); assert_eq!(s.on_tick(), TickDecision::RequestReschedule); assert!(s.take_reschedule()); }
    #[test] fn interrupt_bridge_records_ticks() { let before = interrupt_ticks(); let _ = interrupt_tick(); assert_eq!(interrupt_ticks(), before + 1); assert!(pending_interrupt_requests() > 0); assert!(consume_interrupt_request()); }
    #[test] fn zero_frequency_is_safe() { let timer = Timer::new(0); timer.tick(); assert_eq!(timer.elapsed_ms(), 0); }
}
