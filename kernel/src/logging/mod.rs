...
impl<const N: usize> LogRing<N> {
    pub const fn new() -> Self {
        Self { entries: [None; N], next: 0, len: 0, dropped: 0 }
    }
    ...
    pub fn push(&mut self, record: LogRecord) {
        if N == 0 { self.dropped = self.dropped.saturating_add(1); return; }
        if self.len == N { self.dropped = self.dropped.saturating_add(1); }
...
mod tests {
    ...
    fn ring_is_bounded_and_ordered() {
        ...
        let a = LogRecord { timestamp: 1, level: Level::Info, subsystem: 1, code: 10, value: 1 };
        let b = LogRecord { timestamp: 2, level: Level::Warn, subsystem: 1, code: 11, value: 2 };
        let c = LogRecord { timestamp: 3, level: Level::Error, subsystem: 1, code: 12, value: 3 };
