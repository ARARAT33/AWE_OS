...
fn parse_multiboot2(addr: usize) -> BootInfo {
    ...
    BootInfo { magic: AWE_BOOT_MAGIC, version: awe_boot_protocol::AWE_BOOT_VERSION,
        ...
        framebuffer_address, framebuffer_size, framebuffer_width, framebuffer_height, framebuffer_pitch,
        acpi_rsdp, device_tree: 0, kernel_base: 0, kernel_size: 0 }
}

#[cfg(not(test))]
fn serial_init() {
    unsafe { core::arch::asm!(
        "mov dx, 0x3F9", "xor al, al", "out dx, al", "mov dx, 0x3FB", "mov al, 0x80", "out dx, al",
        "mov dx, 0x3F8", "mov al, 3", "out dx, al", "mov dx, 0x3FB", "mov al, 3", "out dx, al",
        "mov dx, 0x3FA", "mov al, 0xC7", "out dx, al", "mov dx, 0x3FC", "mov al, 0x0B", "out dx, al",
        out("al") _, out("dx") _, options(nostack, preserves_flags)); }
}

#[cfg(not(test))]
fn serial_write(bytes: &[u8]) {
    for &byte in bytes { unsafe { core::arch::asm!(
        "mov dx, 0x3FD", "2: in al, dx", "test al, 0x20", "jz 2b", "mov dx, 0x3F8", "mov al, cl", "out dx, al",
