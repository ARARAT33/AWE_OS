...
impl ReferenceDevice {
    pub const fn block(queue: VirtioQueue) -> Self {
        Self {
            kind: VirtioKind::Block,
            queue,
        }
    }
    pub const fn network(queue: VirtioQueue) -> Self {
        Self {
            kind: VirtioKind::Network,
            queue,
        }
    }
    pub const fn input(queue: VirtioQueue) -> Self {
        Self {
            kind: VirtioKind::Input,
            queue,
        }
    }
    pub const fn gpu(queue: VirtioQueue) -> Self {
        Self {
            kind: VirtioKind::Gpu,
            queue,
        }
...
pub const fn validate_block_request(
    request: BlockRequest,
    capacity_sectors: u64,
) -> Result<(), ReferenceError> {
    if request.sectors == 0 {
        return Err(ReferenceError::ZeroLength);
    }
    let end = request.sector.saturating_add(request.sectors as u64);
    if end > capacity_sectors {
        return Err(ReferenceError::InvalidRange);
    }
    Ok(())
}

pub const fn validate_display_rect(
    rect: DisplayRect,
    width: u16,
    height: u16,
) -> Result<(), ReferenceError> {
...
mod tests {
    ...
    fn block_request_is_bounds_checked() {
        ...
        assert_eq!(
            ...
            Err(ReferenceError::InvalidRange)
        );
    }
    ...
    fn display_rect_is_bounds_checked() {
        ...
        assert_eq!(
            validate_display_rect(rect, 20, 20),
            Err(ReferenceError::InvalidRange)
        );
    }
