...
/// Deterministic lifecycle transition gate. The service supervisor remains in
/// userspace; this is only the state-machine contract.
...
pub const fn lifecycle_maps_to_state(step: DriverLifecycle) -> DriverState {
    match step {
        ...
        DriverLifecycle::Run
        | DriverLifecycle::Resume
        | DriverLifecycle::Recover => DriverState::Running,
        DriverLifecycle::Suspend | DriverLifecycle::Stop => DriverState::Stopping,
        DriverLifecycle::Remove => DriverState::Stopped,
    }
}
