...
pub const fn transition(
    current: DriverLifecycle,
    next: DriverLifecycle,
    trusted: bool,
...
pub const fn lifecycle_maps_to_state(step: DriverLifecycle) -> DriverState {
    match step {
        DriverLifecycle::Discover
        | DriverLifecycle::Identify
        | DriverLifecycle::Probe
        | DriverLifecycle::Bind => DriverState::Discovered,
        DriverLifecycle::Initialize => DriverState::Starting,
        DriverLifecycle::Run | DriverLifecycle::Resume | DriverLifecycle::Recover => {
            DriverState::Running
        }
...
mod tests {
    ...
    fn lifecycle_accepts_only_declared_transitions() {
        assert_eq!(
            transition(DriverLifecycle::Discover, DriverLifecycle::Identify, true),
            Ok(())
        );
        assert_eq!(
            transition(DriverLifecycle::Discover, DriverLifecycle::Run, true),
            Err(LifecycleError::InvalidTransition)
        );
    }
    ...
    fn untrusted_driver_cannot_enter_execution_path() {
        assert_eq!(
            transition(DriverLifecycle::Initialize, DriverLifecycle::Run, false),
    ...
    fn lifecycle_state_mapping_is_deterministic() {
        assert_eq!(
            lifecycle_maps_to_state(DriverLifecycle::Initialize),
            DriverState::Starting
        );
        assert_eq!(
            lifecycle_maps_to_state(DriverLifecycle::Run),
            DriverState::Running
        );
        assert_eq!(
            lifecycle_maps_to_state(DriverLifecycle::Stop),
            DriverState::Stopping
        );
        assert_eq!(
            lifecycle_maps_to_state(DriverLifecycle::Remove),
            DriverState::Quarantined
        );
    }
}
