...
mod tests {
    ...
    #[test]
    fn exposes_bounded_sections() {
        let b = package();
        let h = validate_awos(&b).unwrap();
        let (m, c, d, s) = package_parts(&b, h).unwrap();
        assert_eq!((m.len(), c.len(), d.len(), s.len()), (4, 8, 2, 64));
    }
    #[test]
    fn rejects_unknown_flags() {
        let mut b = package();
        b[28..32].copy_from_slice(&0x8000_0000u32.to_le_bytes());
        assert_eq!(validate_awos(&b), Err(AwosError::UnknownFlags));
    }
    #[test]
