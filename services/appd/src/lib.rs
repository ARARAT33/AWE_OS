...
pub fn validate_manifest(manifest: AppManifest) -> Result<(), AppError> {
    if manifest.id.0 == 0
        || manifest.abi_major != AWE_APP_ABI_MAJOR
        || manifest.abi_minor > AWE_APP_ABI_MINOR
        || manifest.memory_limit_pages == 0
    {
        return Err(AppError::InvalidManifest);
    }
    if manifest.dependency_count as usize > MAX_DEPS {
        return Err(AppError::TooManyDependencies);
    }
    if manifest.resource_count as usize > MAX_RESOURCES {
        return Err(AppError::TooManyResources);
    }
    Ok(())
}
...
pub fn validate_package(header: PackageHeader, total_len: usize) -> Result<(), AppError> {
    if header.version == 0
        || header.manifest_len as usize > MAX_MANIFEST
        || header.payload_len as usize > MAX_PAYLOAD
        || (header.signature_len as usize) < MIN_SIGNATURE
    {
        return Err(AppError::InvalidManifest);
    }
...
mod tests {
    ...
    fn rejects_short_signature() {
        ...
        assert_eq!(
            validate_package(h, PACKAGE_HEADER_SIZE + 1 + 1 + 31),
            Err(AppError::InvalidManifest)
        );
    }
