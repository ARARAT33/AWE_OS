...
impl UpdateManager {
    ...
    pub const fn state(&self, slot: Slot) -> SlotState {
    ...
    pub fn stage(&mut self, slot: Slot, manifest: UpdateManifest) -> Result<(), UpdateError> {
        ...
        if self.states[i] == SlotState::Quarantined {
            return Err(UpdateError::Quarantined);
        }
        if !transition(self.states[i], SlotState::Staged) {
    ...
    pub fn mark_healthy(&mut self, slot: Slot) -> Result<(), UpdateError> {
        ...
        let manifest = match self.manifests[i] {
            Some(m) => m,
            None => return Err(UpdateError::InvalidManifest),
        };
        ...
        if self.states[old] == SlotState::Healthy {
            self.states[old] = SlotState::Staged;
        }
        Ok(())
    }
...
mod tests {
    ...
    fn atomic_ab_update_promotes_only_healthy_slot() {
        let mut m = UpdateManager::new(1);
        m.stage(Slot::B, manifest(2)).unwrap();
        assert_eq!(m.boot_pending().unwrap(), Slot::B);
        m.mark_healthy(Slot::B).unwrap();
    ...
    fn failed_update_rolls_back() {
        let mut m = UpdateManager::new(1);
        m.stage(Slot::B, manifest(2)).unwrap();
        m.boot_pending().unwrap();
        m.mark_failed(Slot::B).unwrap();
        m.rollback(Slot::B).unwrap();
        assert_eq!(m.active(), Slot::A);
    }
    ...
    #[test]
    fn transaction_recovery_is_fail_closed() {
        let mut m = UpdateManager::new(4);
        let tx = Transaction::begin(&m, Slot::B, manifest(5)).unwrap();
        m.stage(Slot::B, manifest(5)).unwrap();
        m.boot_pending().unwrap();
        tx.recover(&mut m).unwrap();
        assert_eq!(m.active(), Slot::A);
        assert_eq!(m.state(Slot::B), SlotState::Failed);
    }
}
