...
pub const fn validate_group(group: GroupId) -> Result<(), IdentityError> {
    if group.0 == u32::MAX {
        Err(IdentityError::InvalidGroup)
    } else {
        Ok(())
    }
}
...
pub const fn authorize(credential: Credential, required: u64) -> Result<(), IdentityError> {
    ...
    if credential.capability_mask & required == required {
        ...
        Err(IdentityError::CapabilityDenied)
    }
}
...
impl GroupSet {
    ...
    pub fn add(&mut self, group: GroupId) -> Result<(), IdentityError> {
        ...
        if self.len == MAX_GROUPS {
            return Err(IdentityError::TooManyGroups);
        }
