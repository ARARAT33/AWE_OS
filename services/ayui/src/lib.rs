...
mod tests {
    ...
    fn font_rendering_and_glyph_drawing_work() {
        ...
        assert_eq!(*glyph_lower_a, [0x00, 0x00, 0x3C, 0x06, 0x3E, 0x66, 0x3E, 0x00]);

        let glyph_num_5 = get_glyph(b'5');
        assert_eq!(*glyph_num_5, [0x7E, 0x60, 0x7C, 0x06, 0x06, 0x66, 0x3C, 0x00]);

        let glyph_symbol = get_glyph(b'>');
        assert_eq!(*glyph_symbol, [0x30, 0x18, 0x0C, 0x06, 0x0C, 0x18, 0x30, 0x00]);
