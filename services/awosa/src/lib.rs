...
pub const CAP_KNOWN_MASK: u64 = CAP_FS_READ | CAP_FS_WRITE | CAP_NET | CAP_IPC | CAP_DEVICE | CAP_UI;
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum RuntimeError { IncompatibleAbi, InvalidArgument, CapabilityDenied, ResourceExhausted, NotFound, AlreadyExists, UnknownCapability }
...
pub const fn validate_capabilities(mask: u64) -> Result<(), RuntimeError> {
    if mask & !CAP_KNOWN_MASK != 0 { Err(RuntimeError::UnknownCapability) } else { Ok(()) }
}
...
pub const fn required_capability(kind: IoKind) -> u64 {
    match kind { IoKind::Read => CAP_FS_READ, IoKind::Write => CAP_FS_WRITE, IoKind::Message => CAP_IPC }
}

pub const fn validate_io(kind: IoKind, size: usize, capabilities: u64) -> Result<(), RuntimeError> {
    if validate_capabilities(capabilities).is_err() { return Err(RuntimeError::UnknownCapability); }
...
impl HandleTable {
    ...
    pub fn allocate(&mut self) -> Result<u16, RuntimeError> {
        let mut i = 0; while i < MAX_HANDLES { if !self.used[i] { self.used[i] = true; return Ok(i as u16); } i += 1; }
        Err(RuntimeError::ResourceExhausted)
    }
    pub fn release(&mut self, handle: u16) -> Result<(), RuntimeError> {
        let index = handle as usize; if index >= MAX_HANDLES || !self.used[index] { return Err(RuntimeError::NotFound); }
        self.used[index] = false; Ok(())
    }
    pub const fn contains(&self, handle: u16) -> bool { let index = handle as usize; index < MAX_HANDLES && self.used[index] }
}
...
mod tests {
    ...
    #[test] fn accepts_compatible_minor() { assert_eq!(negotiate(AbiVersion { major: 1, minor: 2 }).unwrap().major, 1); }
    #[test] fn rejects_unknown_capability_bits() { assert_eq!(validate_capabilities(1 << 63), Err(RuntimeError::UnknownCapability)); }
    #[test] fn rejects_write_without_capability() { assert_eq!(validate_io(IoKind::Write, 128, CAP_FS_READ), Err(RuntimeError::CapabilityDenied)); }
    #[test] fn accepts_bounded_authorized_read() { assert!(validate_io(IoKind::Read, 4096, CAP_FS_READ).is_ok()); }
    #[test] fn rejects_zero_sized_io() { assert_eq!(validate_io(IoKind::Message, 0, CAP_IPC), Err(RuntimeError::ResourceExhausted)); }
    #[test] fn handle_table_is_bounded_and_reusable() { let mut table = HandleTable::new(); let h = table.allocate().unwrap(); assert!(table.contains(h)); assert_eq!(table.release(h), Ok(())); assert!(!table.contains(h)); assert_eq!(table.release(h), Err(RuntimeError::NotFound)); }
}
